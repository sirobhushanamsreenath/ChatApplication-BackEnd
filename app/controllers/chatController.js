...
let markChatAsSeen = (req, res) => {
  ...
  let modifyChat = () => {
    return new Promise((resolve, reject) => {
      ...
      ChatModel.update(findQuery, updateQuery, { multi: true }).exec(
        (err, result) => {
          if (err) {
            console.log(err);
            logger.captureError(
