    ...
    if(req.params.authToken || req.query.authToken || req.body.authToken || req.header('authToken')){
        authModel.findOne({authToken : req.params.authToken || req.body.authToken || req.query.authToken || req.header('authToken')}, (err, authDetails) => {
            if(err){
                ...
                logger.captureError('User is not authorized','middleWares : isAuthorized',5);
                let apiResponse = response.generate(true,'Invalid Or Expired Authorization Key',500, null);
                res.send(apiResponse);
            } else{
                //Verify
        ...
        logger.captureError('Authorization Token Missing','Authorization MiddleWare', 5);
